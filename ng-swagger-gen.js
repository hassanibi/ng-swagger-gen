...
function processResponses(def, path, models) {
    ...
    for (var code in responses) {
        ...
        var type;
        if (!response.schema) {
            type = processContentTypes(def, path, models);
            if (!type) {
                continue;
            }
        } else {
            type = propertyType(response.schema);
        }
...
/*
 * Process an operation's 'produces' section. Returns the content type
 * of result produced by the operation.
 */
function processContentTypes(def, path, models) {
    var contentTypes = def.produces || [];
